...
check("admin publication page exposes only pilot-scoped state-gated publish actions",()=>{assert.match(reviewPage,/disabled=\{mutationPending\|\|!eligibleCurrent\|\|!pilotAuthorized\}[\s\S]*>Publish<\/button>/);assert.match(reviewPage,/disabled=\{mutationPending\|\|!publishedCurrent\}[\s\S]*>Unpublish<\/button>/);assert.match(reviewPage,/Not authorized for current publication pilot/);assert.match(reviewPage,/confirmation!=="PUBLISH"/);assert.equal(/bulk publish|publish all|publish-all/i.test(reviewPage),false)});
...
check("duplicate rapid submission is blocked through the shared per-station mutation lock before the review RPC",()=>{const duplicateGuard=reviewFunction.indexOf("pendingMutationRef.current.has(station.id)");const beginGuard=reviewFunction.indexOf("if(!beginStationMutation(station.id))return");const rpcIndex=reviewFunction.indexOf('rpc("admin_review_station_publication"');assert.ok(duplicateGuard>=0&&duplicateGuard<rpcIndex);assert.ok(beginGuard>=0&&beginGuard<rpcIndex);assert.match(reviewPage,/function beginStationMutation\(stationId:string\)[\s\S]*pendingMutationRef\.current\.add\(stationId\)/);assert.match(reviewPage,/function endStationMutation\(stationId:string\)[\s\S]*pendingMutationRef\.current\.delete\(stationId\)/)});
